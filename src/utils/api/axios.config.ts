...
const instanceWithOptionalAuthorization = axios.create({
	baseURL: BASE_URL,
	headers: {
		Authorization: `Bearer ${localStorage.getItem('ACCESS_TOKEN')}`
	}
...
instanceWithOptionalAuthorization.interceptors.response.use(
	result => {
...
instanceWithOptionalAuthorization.interceptors.request.use(async req => {
...
export {
	instanceDefault,
	instanceWithOptionalAuthorization,
