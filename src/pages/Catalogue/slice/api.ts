...
import type { Filter, Pagination } from '@shared/models/MovieModel'
...
export const fetchMovies =
	(pagination: Pagination) => async (dispatch: Dispatch) => {
...
export const fetchMoviesWithFilter =
	(filter: Filter, pagination: Pagination) => async (dispatch: Dispatch) => {
		try {
			const { current, size } = pagination
			const response = await instanceDefault.get('/movie/catalogue', {
				params: {
